...
class DefineDataset:
    ...
    def assemble_dataset(self):
        ...
        if self.extrema_values is not None:
            print(self.extrema_values, self.parameters_values)

            if self.parameters_values is not None:
                self.extrema_values = torch.cat([self.extrema_values, self.parameters_values ],0)
                print(self.extrema_values)

            extrema_0 = self.extrema_values[:, 0]
            extrema_f = self.extrema_values[:, 1]

            x_coll = self.generator_points(self.n_collocation, self.input_dimensions, self.random_seed, "grid")
            y_coll = torch.from_numpy(np.array([np.nan for _ in range(self.n_collocation)]).reshape(-1, 1)).type(torch.FloatTensor)
            for i in range(1, self.output_dimension):
                y_coll = torch.cat([y_coll, torch.from_numpy(np.array([np.nan for _ in range(self.n_collocation)]).reshape(-1, 1)).type(torch.FloatTensor)], 1)
            ...
            for i in range(self.time_dimensions, self.time_dimensions + self.space_dimensions):
                ...
                x_boundary_0 = self.generator_points(self.n_boundary, self.input_dimensions, self.random_seed, "sobol")
                ...
                x_boundary_1 = self.generator_points(self.n_boundary, self.input_dimensions, self.random_seed, "sobol")
                x_boundary_1[:, i] = torch.tensor(()).new_full(size=(self.n_boundary,), fill_value=1.0)
                x_boundary_1_wo_i = np.delete(x_boundary_1, i, 1)

                [y_boundary_1, type_BC] = Ec.list_of_BC[i - self.time_dimensions][1](x_boundary_1_wo_i * (val_f - val_0) + val_0)
                BC_01.append(type_BC)
            ...
            x_time_0 = self.generator_points(self.n_initial, self.input_dimensions, self.random_seed, "sobol")
    ...
    def generator_points(self, samples, dim, random_seed, type):
        ...
        if type == "random":
            ...
            if dim == 2:
                ...
                enhance = 30. #spatial 5 times than time for Hilbert trans; 4. for single_peri; 30. for double
                sqrt_samples_2 = int(np.sqrt(enhance * samples / ratio))

                #ensure sqrt_samples_2 is odd
                if sqrt_samples_2 % 2 == 0:
                    sqrt_samples_2 += 1
                assert sqrt_samples_2 % 2 == 1

                dir2 = np.linspace(0, 1, sqrt_samples_2)
                sqrt_samples_1 = int(samples / sqrt_samples_2)
                dir1 = np.linspace(0, 1, sqrt_samples_1)

                #update collocation number
                self.n_collocation = sqrt_samples_1 * sqrt_samples_2

                print("\n################################################")
                print("Cartesian grid:", sqrt_samples_1, sqrt_samples_2, ratio)
                print("################################################")
